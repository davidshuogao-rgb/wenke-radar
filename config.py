...
ENABLED_COMPANIES = {
    ...
    "泡泡玛特": False,    # 校招未开时为 0 岗位，开启后自动出现
    "欧莱雅": False,      # 百库平台；2027秋招官网未开时为 0，开启后自动抓取
    "农夫山泉": False,    # 养生堂·农夫山泉，Moka平台（快消行销/生产管培大户）
    "滴滴": True,        # Moka平台（响应加密已破解）
    "名创优品": False,    # 北森平台（快消/潮流零售）
    # --- 快消（新增第二批）---
    "蒙牛": False,        # 北森平台；两季之间校招暂关(0岗)，官网重开自动出现
    "伊利": False,        # 百库旧版(/wt/)接口，动态令牌；管培/销售/职能大户
    "元气森林": False,    # 飞书ATS多租户(jobs.feishu.cn/352020)
    # --- 文化艺术机构（新增） ---
    "UCCA": False,        # UCCA当代艺术中心（含策展/展览类岗位与实习）
    # --- 快消/零售第三批（北森平台）---
    "保利发展": False,    # 北森；校招策划管培/运营管理/企业管理（对口运营/增长营销）
    "青岛啤酒": False,    # 北森；校招菁英计划（销售/国贸/职能）
    # --- 聚合平台（新增） ---
    "牛客日程": False,    # 牛客网校招日程，覆盖2万+公司的校招项目（公司级信息）
    # --- 社招源（已毕业求职者社招机会更多；豁免届别/实习过滤）---
    "美团(社招)": False,       # 美团社招 jobType=3
    "泡泡玛特(社招)": False,   # 消费品牌社招
    "名创优品(社招)": False,   # 消费品牌社招
    "蒙牛(社招)": False,       # 快消社招（北森 category=1）
    "字节跳动(社招)": False,   # 大体量源，daily_only 只报每日新增
    "腾讯(社招)": False,       # 大体量源，daily_only 只报每日新增
    "滴滴(社招)": False,       # talent.didiglobal.com 自研接口（明文JSON）
    "伊利(社招)": False,       # 百库旧版 recruitType=2（快消社招大户，91页）
    "小红书(社招)": False,     # recruitType=social（互联网大厂社招，889岗）
    "小米(社招)": False,       # 飞书ATS portal_type=2（同字节社招套路）
    "B站(社招)": False,        # X-Channel=society（互联网大厂社招）
    "京东(社招)": False,       # zhaopin.jd.com 老JSP站，form编码接口
    # --- 快消/零售第三批社招（北森 category=1）---
    "青岛啤酒(社招)": False,   # 数字化/推广/业务代表
    "喜茶(社招)": False,       # 社招大户（4千+，含产品策划/品牌/设计）；page_size=200
    "维达(社招)": False,       # 数字营销/数据分析
    "统一(社招)": False,       # 社招大户（8百+，含推广/经销/市场）；page_size=200
    # 示例：无官方API、只在猎聘招人的公司（按需替换或停用）
    "宽创国际": False,         # 博物馆展陈/文物IP/策展（策展方向靶心）
    "凯谛思": False,           # Arcadis，文化遗产/考古外企
}
...
CATEGORY_KEYWORDS = ["产品", "运营", "商家", "行业", "营销", "增长", "分析", "战略", "项目"]
